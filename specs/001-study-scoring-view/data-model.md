...
*(Revised 2026-09-19: dropped the slow-load timer and the retry action — see
[research.md § R7 (amended)](research.md#r7-slow-loads-and-a-viewer-that-stops-responding-timing-values).
The state machine still exists, only to drive the form panel; the viewer column carries no
host-added UI of its own.)*
...
| --- | --- | --- | --- |
| `loading` | iframe mounted, study not yet on screen | iframe only | waiting state |
| `loaded` | first image of the study rendered | iframe | placeholder "Scoring is not available yet", no inputs |
| `failed(reason)` | the source has no such study, or is unreachable | iframe only (whatever OHIF itself renders) | unavailable state |

`failed` is terminal: there is no in-app retry. Reloading the page (FR-009) starts over.

Transition rules:

- **No timeout-based failure:** there is no timer. A viewer that never starts, a study with no
  viewable images, or a source that hangs all stay in `loading` indefinitely (research R7).
...
- **Logging:** every transition is logged at `info` with `{ from, to, reason? }`. The `reason`
  is logged for diagnosis even though the UI no longer shows it. No patient data.

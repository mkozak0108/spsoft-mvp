...
| 18 | Open the page with no study saved yet (a fresh study link) | The form starts empty, with no warning in the console, and Session Storage has no key for this study until you press **Add measurement** | US1-4, SC-006, R10 |
| 19 | Close the tab, open the link again in a new tab | The form is empty: closing the tab ended the work | Assumptions |
| 20 | Finish two measurements. In Session Storage, change the second row's `referencedImageId` to a well-formed but wrong value (change one character of the instance UID in it). Reload | Row 2 shows **Not restored** with its saved value and an **Activate** button; the total leaves it out; its ellipse is not drawn; row 1 and its ellipse are back as normal. The viewer logs one `warn` (**verify** R13: a wrong image fails in OHIF's mapping). **Activate** row 2 and draw: it is "Done" with the new area and the total includes it. Reload again: both rows come back as ordinary restored rows | US2-6, FR-017, SC-010, R13 |
| 21 | In the scoring page's console, count changes to this study's stored value: `k = Object.keys(sessionStorage)[0]; n = 0; last = sessionStorage.getItem(k); setInterval(() => { const v = sessionStorage.getItem(k); if (v !== last) { n++; last = v; } }, 50)`. Drag a handle continuously for ten seconds, release, wait two seconds, then read `n` | About ten to twelve changes — one a second plus the edges — not hundreds (**verify** R10: the throttle) | FR-002, R10 |

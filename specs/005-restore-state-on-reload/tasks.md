...
row before anything can be saved, so that is the foundational phase. US1 then adds the store, the
restore command and the viewer's side of it. US2 and US3 need no new code — the design makes them
fall out of US1 — and are their own manual checks.
...
The stored value is fixed by [contracts/saved-state.md](contracts/saved-state.md) and the messages
by [contracts/bridge-messages.md](contracts/bridge-messages.md); follow them exactly. Comments
...
- [ ] T005 In `apps/scoring-form/src/lib/bridge.ts`, require the geometry in `isBridgeEventMessage`: `case BridgeEvent.MeasurementAdded` and `case BridgeEvent.MeasurementUpdated` also need `isEllipseGeometry(payload.ellipse)`, in both `change` cases. Nothing else about the guard changes. Depends on T004
...
**Goal**: After a reload, the form shows the same rows, numbers, statuses, values and total, and the same ellipses are back on the images they were drawn on.

**Independent Test**: quickstart scenarios 1, 2, 7, 8, 12–15, 18.

### Implementation for User Story 1

- [ ] T009 [US1] In a new `apps/scoring-form/src/lib/savedState.ts`, put the store behind `load` and `save`, exactly as in [contracts/saved-state.md](contracts/saved-state.md). Depends on T004, T006:
  ...
  - `load(studyInstanceUid)` returns a `MeasurementState` or `undefined`: a missing key returns `undefined` with no log; anything else that fails logs `logger.warn` with its `reason` **and nothing else**, removes the key, and returns `undefined`. In order: reading or `JSON.parse` throws → `Unreadable` (a `sessionStorage` access that throws → `StorageUnavailable`); `version !== SavedStateVersion.V1` → `UnsupportedVersion`; then every field, → `Shape`. Checks: `nextRowNumber` a positive integer; each row's `id` equal to `` `row-${number}` ``; `number` a positive integer below `nextRowNumber`; `status` a known `RowStatus`; `value`, when present, "a finite `area` ≥ 0 and a `unit` of at most 16 characters"; `ellipse`, when present, `isEllipseGeometry` (T004). It is all or nothing: one bad field drops the whole value;
  - on success, return `{ rows, nextRowNumber, viewerReady: false }` with every `RowStatus.Drawing` row mapped to `RowStatus.Pending` — its ellipse was never finished (FR-007) — and a comment saying so;
  - `save(studyInstanceUid, state)` writes `{ version: SavedStateVersion.V1, nextRowNumber, rows }`, never `viewerReady`. A throw is caught, logged `warn` with `StorageUnavailable`, and swallows nothing else: the caller carries on;
  - the module does not import React and holds no state of its own
- [ ] T010 [US1] In `apps/scoring-form/src/lib/measurements.ts`, start from the saved state and keep it saved. Depends on T009:
  - `useMeasurements` gets its initial state lazily from `savedState.load(studyInstanceUid) ?? INITIAL_MEASUREMENT_STATE`, through `useReducer`'s third argument, so storage is read once per mount and never during a render;
  - add an effect that calls `savedState.save(studyInstanceUid, state)` whenever `state.rows` or `state.nextRowNumber` changes — no throttle and no debounce (research R10), with a comment saying why: it is what makes a change made just before a reload come back;
  - removing the last row still saves `rows: []`, so the numbering does not restart (FR-004). Do not delete the key on an empty list
- [ ] T011 [US1] (fork) In `apps/viewer/extensions/bridge/src/watchMeasurements.ts`, accept `RESTORE_MEASUREMENTS` ([contracts/bridge-messages.md § Receiver rules (viewer), additions](contracts/bridge-messages.md#receiver-rules-viewer-additions)). Depends on T002:
...
- [ ] T012 [US1] (fork) In the same file, put the ellipses back (research R5, R6, [data-model.md § Viewer bridge state](data-model.md#viewer-bridge-state-viewer)). Depends on T011:
  ...
  - it returns the measurement uid, or nothing when the mapping failed (the image is not in this study): on nothing, `log.warn` and skip that one — the host keeps the row with its saved value. On a uid, `links.set(uid, { rowId, last: undefined, lastEllipse: ellipse })`, so a restored ellipse is from then on exactly a drawn one;
  ...
  - `pendingRowId` is not touched, and nothing is posted to the host: each restored ellipse announces its area through the update cornerstone fires once it has recomputed the stats (research R8)
- [ ] T013 [US1] In `useMeasurements` (`apps/scoring-form/src/lib/measurements.ts`), send the restore on every `VIEWER_READY` (research R7). Depends on T010, T011:
  - in the `BridgeEvent.ViewerReady` case, before or after the existing dispatch, collect `stateRef.current.rows` that have an `ellipse` as `{ rowId: row.id, ellipse: row.ellipse }` and, when the list is not empty, `postToViewer` a `buildCommand(BridgeCommand.RestoreMeasurements, { StudyInstanceUID: studyInstanceUid, measurements })`;
...
- [ ] T014 [US1] Run `npm run typecheck && npm run lint` in `apps/scoring-form`, then start both apps as in [quickstart.md § Setup](quickstart.md#setup) and run scenarios 1, 2, 7, 8, 12, 13, 14, 15 and 18. Record the **verify** outcomes for R5 (a restored ellipse is visible, in the right place and editable) and R8 (the row's value comes from the viewer's own recomputation) as a short "Verified 2026-…: …" line under each item in [research.md](research.md). Fix what fails in the module responsible
- [ ] T015 [US1] Commit the fork changes (T011, T012) in `apps/viewer`, push `005-restore-state-on-reload`, and commit in the parent repo the gitlink bump together with `savedState.ts` and `measurements.ts`, staged explicitly
...
**Goal**: A restored ellipse edits and deletes exactly like one drawn in this session, and the next reload reflects both.

**Independent Test**: quickstart scenarios 3, 4, 5, 6, 9.

### Implementation for User Story 2

No new code: T012's link makes a restored ellipse identical to a drawn one for 004's update and
removal paths, and T010's save effect writes every change they cause. T003's widened change test
is what carries a move with no resize.

- [ ] T016 [US2] Run [quickstart.md](quickstart.md) scenarios 3, 4, 5, 6 and 9. Record the **verify** outcomes for R4 and R10 (a move with no resize is reported and saved, and the higher message and save rate leaves both apps responsive through a ten-second drag) in [research.md](research.md). If an edit or a deletion does not reach the right row, fix it in `apps/viewer/extensions/bridge/src/watchMeasurements.ts`, record the finding in research R6, and commit and push the fork and the gitlink bump as in T015

**Checkpoint**: Restored work behaves like work made in this session.
...
- [ ] T018 Update `ARCHITECTURE.md` (FR-016, SC-009), keeping `messages.ts` as the stated source of truth:
  - **Flow**: add a restore diagram — page reloads → rows read from the tab's store → `VIEWER_READY` → `RESTORE_MEASUREMENTS` → ellipses redrawn and relinked → `MEASUREMENT_UPDATED` with the recomputed area;
  - **Viewer → host table**: add `ellipse` to `MEASUREMENT_ADDED` and `MEASUREMENT_UPDATED`, and change when `MEASUREMENT_UPDATED` is sent to "the area, the unit or the shape changed";
  - **Host → viewer table**: add `RESTORE_MEASUREMENTS`, its payload and its study check;
  - **Why every message carries a version**: add that the new field and the new command stayed on `V1`, and why (research R12);
  - **Receiver rules**: the host's ellipse check with its limits; the viewer's per-command branch, the 100-entry cap and the study check;
  - **Where the state lives**: replace "Nothing is saved: reloading the page starts with no rows" with the store — `sessionStorage`, one key per study, what is in it, its version, that it ends with the tab, and that the viewer still stores nothing;
  - **Key decisions**: the host is the only store and why a second store in the viewer was rejected (R1); `sessionStorage` because the lifetime is the tab; the geometry on the wire instead (R2, R3); the area recomputed rather than restored (R8); OHIF's own `addRawMeasurement` as the restore path (R5); saving on every change with no throttle (R10);
  ...
  - **Known limitations**: replace "after a viewer reload, finished rows keep their values but can no longer be edited" with the narrower ones this feature leaves — a saved ellipse whose image is no longer in the study comes back as a row with a value but no ellipse, which cannot then be edited or deleted; a second tab, another browser and another device each start empty; closing the tab ends the work; a failed save or restore is only visible in the console
- [ ] T019 Comment and enum audit over everything this feature touched (`git diff main` in the parent repo, and `git diff f2ee4fcef6` in `apps/viewer`): remove comments that restate the code, and confirm no domain value is repeated as a string literal (the command name, `SavedStateVersion`, the rejection reasons, the OHIF source name and version)
- [ ] T020 Run the whole [quickstart.md](quickstart.md), scenarios 1–19 in order, from a fresh tab. Then the delivery gates: in `apps/scoring-form`, `npm run typecheck`, `npm run lint`, `npm run build` and `npm audit --omit=dev` (no high or critical); in the fork, `pnpm exec tsc --noEmit -p tsconfig.json` in `apps/viewer`, looking only at errors under `extensions/bridge/`
- [ ] T021 Final pin: the fork branch is pushed, `git submodule status` shows the pushed `005-restore-state-on-reload` commit, and the parent repo's gitlink points at it. Commit T018–T019's changes in the parent repo with small, imperative messages
...
- **US2 (Phase 4)**: after US1 — it checks US1's links and saves with several rows.
...
- Phase 3: T009 → T010 → T013 on the host (T013 also needs T011's command name); fork chain
  T011 → T012. The two sides meet at T014.
...
- Phase 2: the fork chain (T002 → T003) and the host work (T004 → T005, T006) are independent
  until T007.
- Phase 3: the store (T009 → T010) and the viewer's restore (T011 → T012) are independent until
  T013.
...
2. Phase 3 (US1), then **stop and validate** with quickstart scenarios 1, 2, 7, 8, 12–15 and 18.
   The **verify** items — the restore path and its render (R5), and the area arriving by
   recomputation (R8) — are the risky part, so this is where surprises show up.

### Incremental delivery

1. US1 → a reload brings the form and the ellipses back (demo).
2. US2 → restored measurements edit, delete and re-save like any other.
...
- Limits from the contracts are copied into the tasks that implement them (`referencedImageId`
  ...
  characters", `area` "a finite number ≥ 0"), so they are not left to the implementer.
- The store is the first thing this project writes outside memory. Principle II's justification
  ...
  is logged.
- Fork pushes are outward-facing: T008, T015, T016 and T017 push a branch to
  `mkozak0108/Viewers`. Ask the user before the first push (T008).
